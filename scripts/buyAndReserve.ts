...
(async () => {
  const tx1 = await program.methods.buyChipWithSol(new anchor.BN(0.01 * LAMPORTS_PER_SOL))
  .accounts({
  ...
  const tx2 = await program.methods.reserveChips(new anchor.BN(0.01 * LAMPORTS_PER_SOL))
    .accounts({
